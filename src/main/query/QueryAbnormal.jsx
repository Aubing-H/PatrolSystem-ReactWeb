...
import {Button, Descriptions, DatePicker, Select, Image} from 'antd';
...
class QueryAbnormal extends React.Component{
    constructor(props){
        super(props);
        this.state={
            data: [],
    ...
    getData(){
        ...
        axios.post(ipAddress + '/abnormalRecord/queryByInfo', req)
        .then((response) => {
            if(response !== null) {
                console.log("## Query ## " + response.data.msg);
                this.setState({
                    ...
                    {this.state.data.map(item=>{
                        ...
                        <div  key={item.userId + item.time} style={{margin: "5px 0px", borderRadius: 5, padding: 10, background: "#def"}}>
                            <Descriptions title={this.getName(item.userId)}>
                                ...
                                <Descriptions.Item label="异常">{item.abnormalItem}</Descriptions.Item>
                                <Descriptions.Item label="异常描述">{item.abnormalDetail}</Descriptions.Item>
                                <Descriptions.Item label="上传图片">
                                    <Image src={ipAddress + '/image/' + item.pictureLink.substring(37, item.pictureLink.length)}/>
                                </Descriptions.Item>
                            </Descriptions>
                        </div>
                        )
